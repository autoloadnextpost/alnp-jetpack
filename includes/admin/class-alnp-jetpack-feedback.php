<?php
/**
 * Auto Load Next Post: JetPack - Feedback Notice
 *
 ...
 * @category Admin
 * @package  Auto Load Next Post: JetPack/Admin/Notices
 * @license  GPL-2.0+
 */
...
if ( ! class_exists( 'ALNP_JetPack_Feedback' ) ) {
	class ALNP_JetPack_Feedback {
		 ...
		 // Get the years.
			...
			if ( $years > 1 ) {
			 /* translators: Number of years */
				return sprintf( __( '%s years', 'alnp-jetpack' ), $years );
			} elseif ( $years > 0 ) {
				return __( 'a year', 'alnp-jetpack' );
			}
			...
			if ( $weeks > 1 ) {
				...
				return sprintf( __( '%s weeks', 'alnp-jetpack' ), $weeks );
			} elseif ( $weeks > 0 ) {
				return __( 'a week', 'alnp-jetpack' );
			}
			...
			if ( $days > 1 ) {
				...
				return sprintf( __( '%s days', 'alnp-jetpack' ), $days );
			} elseif ( $days > 0 ) {
				return __( 'a day', 'alnp-jetpack' );
			}
			...
			if ( $hours > 1 ) {
				...
				return sprintf( __( '%s hours', 'alnp-jetpack' ), $hours );
			} elseif ( $hours > 0 ) {
				return __( 'an hour', 'alnp-jetpack' );
			}
			...
			if ( $minutes > 1 ) {
				...
				return sprintf( __( '%s minutes', 'alnp-jetpack' ), $minutes );
			} elseif ( $minutes > 0 ) {
				return __( 'a minute', 'alnp-jetpack' );
			}
			...
			if ( $seconds > 1 ) {
				...
				return sprintf( __( '%s seconds', 'alnp-jetpack' ), $seconds );
			} elseif ( $seconds > 0 ) {
				return __( 'a second', 'alnp-jetpack' );
			}
		}
			...
			<div class="notice updated alnp-feedback-notice">
				<div class="alnp-feedback-notice-inner">
					<div class="alnp-feedback-notice-icon">
						...
						<img src="https://ps.w.org/alnp-jetpack/assets/icon-256x256.png" alt="<?php printf( esc_attr__( '%1$s WordPress Plugin', 'alnp-jetpack' ), esc_attr( $this->name ) ); ?>" />
					</div>
					<div class="alnp-feedback-notice-content">
						<?php /* translators: 1. Name */ ?>
						<h3><?php printf( esc_html__( 'Are you enjoying %1$s?', 'alnp-jetpack' ), esc_html( $this->name ) ); ?></h3>
						<p>
							<?php /* translators: 1. Name, 2. Time */ ?>
							<?php printf( esc_html__( 'You have been using %1$s for %2$s now! Mind leaving a quick review to let me know know what you think? I\'d really appreciate it!', 'alnp-jetpack' ), esc_html( $this->name ), esc_html( $time ) ); ?>
						</p>
					</div>
					<div class="alnp-install-now">
						<?php printf( '<a href="%1$s" class="button button-primary alnp-install-button" target="_blank">%2$s</a>', esc_url( ALNP_JETPACK_REVIEW_URL . '#new-post' ), esc_html__( 'Leave a Review', 'alnp-jetpack' ) ); ?>
						<a href="<?php echo esc_url( $no_bug_url ); ?>" class="no-thanks"><?php echo esc_html__( 'No thanks / I already have', 'alnp-jetpack' ); ?></a>
					</div>
				</div>
			</div>
			<?php
...
/*
 * Instantiate the ALNP_JetPack_Feedback class.
 */
new ALNP_JetPack_Feedback(
	array(
		'slug'       => 'alnp_jetpack_plugin_feedback',
		'name'       => __( 'Auto Load Next Post: JetPack', 'alnp-jetpack' ),
